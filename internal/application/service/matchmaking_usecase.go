...
// MatchmakingUseCase 是匹配流程的核心用例，编排票据入队、匹配、确认和结果发布。
type MatchmakingUseCase struct {
	ticketRepo  domainrepo.TicketRepository
	configRepo  domainrepo.ConfigRepository
	publisher   domainrepo.EventPublisher
	matchmakers []domainsvc.Matchmaker
	mu          sync.Mutex
}

// NewMatchmakingUseCase 创建匹配用例实例。
...
func (uc *MatchmakingUseCase) EnqueueAndMatch(ctx context.Context, ticket *entity.Ticket) error {
	// 1. 获取该游戏模式的匹配配置
	...
	// 3. 标记为匹配中并持久化
	ticket.Status = valueobject.TicketStatusMatching
	...
	// 5. 获取匹配池中同模式的等待中票据
	pool, err := uc.ticketRepo.FindByStatus(ctx, ticket.GameMode, valueobject.TicketStatusMatching)
	if err != nil {
	...
	for _, t := range match.Tickets {
		t.Status = valueobject.TicketStatusMatched
...
func (uc *MatchmakingUseCase) finalizeMatch(ctx context.Context, match *entity.Match) error {
	...
	// 将票据标记为已确认
	for _, t := range match.Tickets {
		t.Status = valueobject.TicketStatusConfirmed
		t.UpdatedAt = time.Now()
...
func (uc *MatchmakingUseCase) requestConfirm(ctx context.Context, match *entity.Match, cfg *valueobject.MatchConfig) error {
	// 更新票据状态为确认中
	for _, t := range match.Tickets {
		t.Status = valueobject.TicketStatusConfirming
		t.UpdatedAt = time.Now()
		_ = uc.ticketRepo.Save(ctx, t)
	}
...
func (uc *MatchmakingUseCase) startConfirmTimeout(match *entity.Match, timeout time.Duration) {
	time.Sleep(timeout)
	...
	for _, t := range match.Tickets {
		...
		if current.Status == valueobject.TicketStatusConfirming {
			slog.Warn("确认超时，取消票据", "ticket_id", t.ID, "match_id", match.ID)
			_ = uc.ticketRepo.UpdateStatus(context.Background(), t.ID, valueobject.TicketStatusCancelled)
		}
	}
}

// getMatchmaker 根据游戏模式查找对应的匹配器。
func (uc *MatchmakingUseCase) getMatchmaker(mode valueobject.GameMode) (domainsvc.Matchmaker, error) {
...
func (uc *MatchmakingUseCase) generateMatchID() string {
